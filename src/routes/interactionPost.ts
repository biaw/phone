...
export default async function handleInteractionPost(request: Request, env: Env): Promise<Response> {
  // check and validate headers
  if (!request.headers.get("X-Signature-Ed25519") || !request.headers.get("X-Signature-Timestamp")) return Response.redirect(env.FALLBACK_URL, 307);
  if (!await isValidRequest(request, env.DISCORD_PUBLIC_KEY, PlatformAlgorithm.Cloudflare)) return new Response("", { status: 401 });
  ...
  return commandHandler(interaction, request, env);
}
